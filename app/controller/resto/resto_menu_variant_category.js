...
exports.get = async function (req, res) {
  ...
  try {
    ...
    * ,
    a.status AS status
    FROM resto_menu_variant_category AS a
    WHERE 1+1=2 `;
...
exports.insert = async function (req, res) {
  ...
  try {
    ...
    const require_data = ["resto_menu_variant_category_name"];
    for (const row of require_data) {
      if (!req.body[`${row}`]) {
        data.error = true;
        data.message = `${row} is required!`;
    ...
    var _res = await models.insert_query({
      data: req.body,
      table: "resto_menu_variant_category",
    });
...
exports.update = async function (req, res) {
  ...
  try {
    ...
    const require_data = [
      "resto_menu_variant_category_name",
      "resto_menu_variant_category_id",
    ];
    for (const row of require_data) {
      if (!req.body[`${row}`]) {
        data.error = true;
    ...
    var _res = await models.update_query({
      data: req.body,
      key: "resto_menu_variant_category_id",
      table: "resto_menu_variant_category",
    });
    return response.response(_res, res);
  } catch (error) {
...
exports.delete = async function (req, res) {
  try {
    ...
    var data = { data: req.body };
    const require_data = ["resto_menu_variant_category_id"];
    for (const row of require_data) {
      if (!req.body[`${row}`]) {
        data.error = true;
        data.message = `${row} is required!`;
