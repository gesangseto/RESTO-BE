...
exports.insert = async function (req, res) {
  ...
  try {
    ...
    const require_data = ["resto_menu_category_name"];
    for (const row of require_data) {
      if (!req.body[`${row}`]) {
        data.error = true;
        data.message = `${row} is required!`;
...
exports.update = async function (req, res) {
  ...
  try {
    ...
    const require_data = ["resto_menu_category_name", "resto_menu_category_id"];
    for (const row of require_data) {
      if (!req.body[`${row}`]) {
        data.error = true;
        data.message = `${row} is required!`;
...
exports.delete = async function (req, res) {
  try {
    ...
    var data = { data: req.body };
    const require_data = ["resto_menu_category_id"];
    for (const row of require_data) {
      if (!req.body[`${row}`]) {
        data.error = true;
        data.message = `${row} is required!`;
